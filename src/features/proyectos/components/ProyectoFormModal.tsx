...
export function ProyectoFormModal({
  ...
  const crearProyecto = useStoreRaiz((estado) => estado.crearProyecto);
  const actualizarProyecto = useStoreRaiz(
    (estado) => estado.actualizarProyecto,
  );
  ...
  const manejarEnvio = (evento: React.FormEvent<HTMLFormElement>) => {
    ...
    if (proyecto) {
      ...
      crearProyecto({ nombre: nombreNormalizado, descripcion });
    }
