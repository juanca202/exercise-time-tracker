...
/**
 * Verifica que un Registro de Tiempo tenga una `duracionMinutos` numérica,
 * finita y mayor que cero. Cualquier otro valor (negativo, `NaN`, no
 * numérico) se considera inválido y se excluye del cálculo (AC-002/TC-005).
 */
function tieneDuracionValida(registro: RegistroDeTiempo): boolean {
  const { duracionMinutos } = registro;
  return (
 ...
 *
 * Agrupa por `tareaId` y suma `duracionMinutos`, descartando Registros con
 * `tareaId` ausente o `duracionMinutos` inválida (negativa o no numérica)
 * sin lanzar una excepción. Una Tarea sin Registros válidos simplemente no
...
export function calcularTotalPorTarea(
  ...
  for (const registro of registros) {
    ...
    totales.set(
      registro.tareaId,
      (totales.get(registro.tareaId) ?? 0) + registro.duracionMinutos,
    );
  }
