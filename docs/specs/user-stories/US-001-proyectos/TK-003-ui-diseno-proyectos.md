# TK-003: UI y diseño de la pantalla de Proyectos
...
- [TK-001: Crear, editar y listar Proyectos](TK-001-crear-editar-y-listar-proyectos.md) — componentes funcionales (`project-modal.tsx`, `project-card.tsx`, `projects-list.tsx`, `src/app/proyectos/page.tsx`) sobre los que se aplica esta tarea; TK-003 no agrega comportamiento nuevo, solo estilo y verificación visual.
- [TK-002: Layout, navegación y rutas placeholder (US-000)](../US-000-fundamentos/TK-002-layout-navegacion-y-rutas-placeholder.md) — `AppShell` + `SidebarNav` ya implementan la navegación lateral con el enlace a `/proyectos` (AC-008); esta tarea no modifica ese módulo compartido, solo verifica el resultado (TC-014).
...
- [ ] **IT-06** — Confirmar (sin cambios de código) que la navegación lateral hacia "Proyectos" (AC-008, TC-014) funciona sin ajustes adicionales, heredada de `SidebarNav` (US-000 TK-002).
