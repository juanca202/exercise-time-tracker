...
- [ ] **IT-08** — Escribir `e2e/us-001-project-persistence.spec.ts` (Playwright, ADR-008) cubriendo TC-005: crear un Proyecto (Nombre + Descripción) vía el modal (IT-02/IT-03), recargar completamente la página y verificar que sigue visible en el listado (IT-05) con el mismo Nombre y Descripción.
- [ ] **IT-09** — Extender el mismo spec para cubrir TC-006: crear varios Proyectos, recargar, y verificar que todos persisten sin pérdida ni duplicación.
- [ ] **IT-10** — Verificar (sin código de producto adicional) que `src/app/proyectos/page.tsx` y `ProjectsList` (IT-05) gatean la lectura de `useProjects()` detrás de `useHasHydrated()` antes de dar la tarea por completa: es la garantía de que la recarga no produce hydration-mismatch ni pérdida de datos visibles.

## Observaciones

Sin pendientes documentados. La verificación de persistencia (IT-08 a IT-10) no agrega código de producto nuevo: confirma y traza (AC-003) la garantía de persistencia genérica que US-000 ya entrega para la entidad Proyecto.
