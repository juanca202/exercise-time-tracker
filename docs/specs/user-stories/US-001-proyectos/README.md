...
- En el prototipo Figma, cada tarjeta de Proyecto muestra además el texto "Tiempo registrado" (total acumulado). La misma regla de cálculo (`calculateTotalByProject`) la usa también [US-003 (Historial de registros)](../US-003-historial-de-registros/README.md), AC-003; para no duplicarla ni acoplar esta historia a `features/historial/` (ADR-005), la función vive en el módulo compartido de [US-000 (Fundamentos)](../US-000-fundamentos/README.md) y esta historia (TK-001) la consume directamente desde ahí.
