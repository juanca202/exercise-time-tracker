...
| ----- | ------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **I** | Independiente | Parcial   | Depende de que exista al menos un Proyecto ([US-001](../US-001-proyectos/README.md)) para poder crear Tareas (BR-01). Esta es una dependencia **funcional de dominio**, no de implementación: gracias al store raíz y los tipos de dominio ya definidos en [US-000](../US-000-fundamentos/README.md) (AC-001, AC-003), esta historia puede desarrollarse en paralelo a US-001 sembrando Proyectos de prueba mediante el CRUD crudo del store compartido, sin esperar a que la UI de Proyectos esté terminada. |
...
| **S** | Pequeña       | Parcial   | Agrupa tres capacidades relacionadas (Tareas, temporizador, registro manual) por decisión del usuario; en `work-plan` se planificaron dentro de un mismo `TK-XXX` (en vez de dividirlas) porque las tres operan sobre el mismo CRUD compartido (`Task`/`TimeEntry` vía `useAppStore`, US-000): dividirlas habría introducido dependencias cruzadas entre TKs sin reducir el acoplamiento real.                                                                                                                |
...
| ---------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dependencias listas                | Parcial | Depende funcionalmente de [US-001 (Proyectos)](../US-001-proyectos/README.md) por BR-01 (una Tarea requiere un Proyecto existente); no es una aclaración pendiente ni un bloqueo de implementación, ya que el store raíz de [US-000](../US-000-fundamentos/README.md) permite sembrar Proyectos de prueba y desarrollar esta historia en paralelo. El orden Proyectos → Tareas aplica al flujo de valor para el usuario final, no a la secuencia de desarrollo. |
...
- Por decisión explícita del usuario, esta historia agrupa Gestión de Tareas, Temporizador (RF-005 a RF-010) y Registro manual de tiempo (RF-011 a RF-013), lo que degrada la dimensión INVEST "S" (Pequeña) a Parcial. En `work-plan` se resolvió planificar estas tres capacidades en un único `TK-XXX` (no dividido), ya que las tres son distintas formas de crear/actualizar las mismas entidades (`Task`/`TimeEntry`) a través del mismo CRUD del store compartido (US-000): dividirlas habría fragmentado esa lógica compartida sin bajar el acoplamiento real entre las piezas.
...
- El prototipo Figma también muestra en la pantalla de Tareas un stat card "Total Mensual"; reutiliza la misma regla de cálculo que consume [US-003 (Historial de registros)](../US-003-historial-de-registros/README.md), AC-004 — no se duplica como AC en esta historia. Para no acoplar esta historia a `features/historial/` (ADR-005), esa función (`calculateTotalByMonth`) vive en el módulo compartido de [US-000 (Fundamentos)](../US-000-fundamentos/README.md); la pantalla de Tareas la consume directamente desde ahí, sin depender de que US-003 esté implementada.
...
- La dependencia de esta historia con [US-001 (Proyectos)](../US-001-proyectos/README.md) es de **regla de negocio** (BR-01: una Tarea requiere un Proyecto existente), no de archivos ni de secuencia de desarrollo: al estar el store raíz, los tipos de dominio y el CRUD crudo de Proyecto ya resueltos en [US-000 (Fundamentos)](../US-000-fundamentos/README.md), esta historia no necesita que US-001 esté implementada para empezar — puede sembrar Proyectos de prueba directamente contra el store compartido y avanzar en paralelo.
