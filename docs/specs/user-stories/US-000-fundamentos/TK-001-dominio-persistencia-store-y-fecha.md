...
- [ ] **IT-11** — Implementar `StorageBootstrapper` (client component, sin salida visual) en `storage-bootstrapper.tsx`, que invoca `requestPersistentStorage()` en un `useEffect` al montar. [TK-002: Layout, navegación y rutas placeholder](TK-002-layout-navegacion-y-rutas-placeholder.md) lo monta en el layout raíz.
